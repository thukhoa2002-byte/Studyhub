...
import { normalizeOutputLanguage } from "../prompts/drugExtractionPrompt.js";
...
router.post("/extract-ai", requireDrugImportAdmin, async (req, res) => {
  ...
  try {
    ...
    const sourceMetadata = { ...(req.body?.sourceMetadata || {}), outputLanguage };
    if (req.body?.documentKind === "guideline_table") {
      const result = await extractGuidelineTableWithAi({ text, sourceMetadata, itemType: req.body?.documentItemType || "table", outputLanguage });
    ...
    const result = await extractDrugWithAi({ text, drugName: req.body?.drugName, sourceMetadata, outputLanguage });
