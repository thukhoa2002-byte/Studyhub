      ...
      <div className="glass-panel border border-rose-100 bg-white/75 p-5">
        ...
        {contentLayout && contentBook && isOwner && <div className="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-4">
          ...
          <div className="mt-4 max-h-[75vh] overflow-y-auto rounded-xl bg-slate-100 p-3 sm:p-6">
            {contentLayout.pages.map((page, pageIndex) => <div key={`editor-page-${page.pageNumber}`} className="mx-auto mb-6 max-w-[760px] last:mb-0"><div className="mb-2 text-center text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">Trang {page.pageNumber}</div><div className="min-h-[900px] bg-white px-8 py-10 shadow-[0_2px_12px_rgba(15,23,42,.08)] sm:px-16 sm:py-14">{page.blocks.map((block, blockIndex) => { const heading = block.role === "heading"; const rows = Math.max(1, Math.min(16, block.text.split(/\r?\n/).length + Math.ceil(block.text.length / 105))); return <div key={`content-${page.pageNumber}-${blockIndex}`} className="group relative"><textarea value={block.text} rows={rows} onChange={(event) => updateContentBlockText(pageIndex, blockIndex, event.target.value)} aria-label={`Nội dung trang ${page.pageNumber}, ${roleLabel(block.role)}`} className={`block w-full resize-y overflow-hidden border-0 bg-transparent px-0 py-1 leading-[1.5] text-slate-800 outline-none transition-colors focus:bg-indigo-50/40 ${heading ? "font-bold text-base" : "text-sm"}`} /><span className="pointer-events-none absolute -left-5 top-2 hidden text-[9px] font-bold uppercase tracking-wide text-indigo-400 group-focus-within:block">{roleLabel(block.role)}</span><button type="button" title="Xóa block" aria-label="Xóa block" onClick={() => removeContentBlock(pageIndex, blockIndex)} className="absolute -right-7 top-1 hidden rounded-md p-1 text-rose-500 hover:bg-rose-50 group-hover:block"><Trash2 size={14} /></button></div>; })}</div></div>)}
          </div>
        </div>}
