...
export default function ReferenceLibraryPage({ user, onAiCallsRemaining }: { user: User | null; onAiCallsRemaining?: (remaining: number) => void }) {
  ...
  async function addBook(event: React.FormEvent) { event.preventDefault(); if (!user || !file || !title.trim()) return; setBusy(true); setError(""); try { const layout = await extractReferenceBook(file); setExtraction(layout); await createReferenceBook(user.id, title, author, file); setTitle(""); setAuthor(""); setFile(null); await refreshBooks(); } catch (e) { setError(e instanceof Error ? e.message : "Không thể OCR/lưu sách."); } finally { setBusy(false); } }
  return <>
