    ...
    {section === "guidelines" ? <GuidelineDataPage route={guidelineRoute} onNavigate={onNavigate} onManage={() => onNavigate("/admin/guidelines")} /> : section === "tools" ? <section className="mode-panel mx-auto w-full max-w-[1200px] px-4 py-8 sm:px-6 xl:px-8"><div className="glass-panel border border-teal-100 bg-white/75 p-8 text-center"><h1 className="text-xl font-extrabold text-rose-950">Máy tính y khoa</h1><p className="mt-2 text-sm font-semibold text-slate-500">Chưa có máy tính nào được cấu hình.</p></div></section> : <section className="mode-panel mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 xl:px-8">
      <div className="glass-panel border border-rose-100 bg-white/75 p-5">
        ...
        {isOwner && <form onSubmit={addFolder} className="mt-5 grid gap-3 rounded-2xl border border-dashed border-violet-200 bg-violet-50/40 p-4 sm:grid-cols-[1fr_220px_auto]">
          ...
          <AnimatedDropdown value={newFolderParentId || ""} options={[{ value: "", label: "Thư mục gốc" }, ...folders.map((folder) => ({ value: folder.id, label: folder.title }))]} onChange={(value) => setNewFolderParentId(value || null)} ariaLabel="Thư mục cha mới" triggerClassName="h-11 rounded-xl border border-violet-100 bg-white px-3 py-2.5 text-slate-700" />
        ...
        {isOwner ? <form onSubmit={addBook} className="mt-3 grid gap-3 rounded-2xl border border-dashed border-teal-200 bg-teal-50/40 p-4 sm:grid-cols-[1fr_1fr_140px_220px_auto]">
          ...
          <AnimatedDropdown value={parentId || ""} options={[{ value: "", label: "Thư mục gốc" }, ...folders.map((folder) => ({ value: folder.id, label: folder.title }))]} onChange={(value) => setParentId(value || null)} ariaLabel="Thư mục cha của sách" triggerClassName="h-11 rounded-xl border border-teal-100 bg-white px-3 py-2.5 text-slate-700" />
