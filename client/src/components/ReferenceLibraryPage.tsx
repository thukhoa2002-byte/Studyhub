import { BookOpenCheck, Books, FileUp, Loader2, Trash2 } from "lucide-react";
import type { User } from "@supabase/supabase-js";
import { useEffect, useState } from "react";
import GuidelinesPage from "./GuidelinesPage";
import { createReferenceBook, deleteReferenceBook, listReferenceBooks, type ReferenceBook } from "../services/referenceBooks";
...
export default function ReferenceLibraryPage({ user, onAiCallsRemaining }: { user: User | null; onAiCallsRemaining?: (remaining: number) => void }) {
  ...
  async function addBook(event: React.FormEvent) { event.preventDefault(); if (!user || !file || !title.trim()) return; setBusy(true); setError(""); try { await createReferenceBook(user.id, title, author, file); setTitle(""); setAuthor(""); setFile(null); await refreshBooks(); } catch (e) { setError(e instanceof Error ? e.message : "Không thể lưu sách."); } finally { setBusy(false); } }
  return <>
