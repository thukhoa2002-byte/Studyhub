...
function getPandaRoutine(date: Date): { action: PandaAction; label: string; message: string; emoji: string } {
  ...
  if (minutes >= 6 * 60 + 15 && minutes < 7 * 60 + 30) return { action: "wake", label: "Thức dậy", message: "Panda vừa vệ sinh, ăn sáng xong rồi nè!", emoji: "🌱" };
  if (minutes >= 7 * 60 + 30 && minutes < 11 * 60 + 30) return { action: "eat", label: "Ăn sáng", message: "Đang gặm trúc lấy năng lượng học cùng bạn.", emoji: "🎋" };
  ...
  if (minutes >= 17 * 60 + 30 && minutes < 19 * 60) return { action: "exercise", label: "Tập thể dục", message: "Panda vận động nhẹ cho đầu óc tỉnh táo.", emoji: "🎈" };
  if (minutes >= 19 * 60 && minutes < 23 * 60) return { action: "eat", label: "Ăn tối", message: "Ăn tối xong rồi ôn thêm một chút nha.", emoji: "🎋" };
