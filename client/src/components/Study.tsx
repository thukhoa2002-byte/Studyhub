    ...
    <section className="mx-auto max-w-4xl px-5 py-8 sm:py-12">
      ...
      <article className="glass-card min-h-[430px] overflow-hidden rounded-3xl border border-rose-100 bg-white/95 shadow-[0_18px_45px_rgba(244,114,182,0.12)]">
        ...
        <div className="flex min-h-[325px] flex-col justify-center px-6 py-10 text-center sm:px-14">
          <p className={`mb-5 text-xs font-semibold uppercase tracking-[0.2em] ${showAnswer && isClozeCard ? "text-teal-500" : "text-slate-400"}`}>{showAnswer && isClozeCard ? "Đáp án đầy đủ" : "Câu hỏi"}</p>
          <div className="rich-content text-2xl font-semibold leading-relaxed text-slate-800 sm:text-3xl" dangerouslySetInnerHTML={{ __html: showAnswer && isClozeCard ? toClozeRevealedHtml(question.question) : toClozeQuestionHtml(question.question) }} />
