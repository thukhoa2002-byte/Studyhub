  ...
  return <section className="mb-8 overflow-visible rounded-[2rem] border border-violet-200/80 bg-white/80 p-5 shadow-sm backdrop-blur-xl sm:p-7">
    ...
    {questions.length > 0 && <div className="mt-7 border-t border-violet-100 pt-6">
      ...
      <div className="mt-5 flex flex-wrap justify-end gap-3">
        <button type="button" disabled={busy || !reviewed || !title.trim() || invalidCount > 0} onClick={() => void exportWord(title, questions)} className="inline-flex items-center gap-2 rounded-xl border border-violet-200 bg-white px-4 py-3 text-sm font-bold text-violet-700 disabled:opacity-40"><Download size={17} />Xuất Word .docx</button>
