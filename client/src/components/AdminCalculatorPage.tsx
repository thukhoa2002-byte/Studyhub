...
function PreviewInput({ field, value, onChange }: { field: CalculatorInputField; value: unknown; onChange: (value: unknown) => void }) {
  ...
  return <label className="block text-sm font-bold text-slate-700">{field.label}<select value={String(value ?? "")} onChange={(event) => onChange(field.type === "boolean" ? event.target.value === "true" : event.target.value)} className="mt-1.5 h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"><option value="">Chọn giá trị</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>;
}
