    ...
    <section className="mode-panel mx-auto w-full max-w-[1600px] px-4 py-8 sm:px-6 xl:px-8" aria-labelledby="guidelines-title">
      <div className="glass-panel border border-rose-100/80 bg-white/68 p-4 sm:p-6 xl:p-8">
        ...
        {canManage && showDocumentForm && <form ref={documentFormRef} onSubmit={submitDocument} className="mt-6 grid gap-4 rounded-3xl border border-rose-100 bg-white/75 p-5 sm:grid-cols-2">
          ...
          <FileDropZone id="guideline-main-pdf" name="file" required accept="application/pdf,.pdf" onFiles={() => setPreparedExtraction(null)} className="text-sm font-bold text-slate-700 sm:col-span-2"><span className="block rounded-xl border border-dashed border-teal-200 bg-teal-50/55 px-4 py-4">PDF guideline chính</span></FileDropZone>
          <FileDropZone id="guideline-supplement-pdf" name="supplementFile" accept="application/pdf,.pdf" onFiles={() => setPreparedExtraction(null)} className="text-sm font-bold text-slate-700 sm:col-span-2"><span className="block rounded-xl border border-dashed border-violet-200 bg-violet-50/55 px-4 py-4">PDF Supplementary Data (không bắt buộc)</span><span className="mt-1.5 block text-xs font-medium text-slate-400">Tổng hai file tối đa 40 MB khi dùng AI · mỗi PDF tối đa 40 MB</span></FileDropZone>
