...
export default function GuidelineRecommendationTablesPanel({ guidelineId, user, tables, groups, sections, recommendations, storageReady, setTables, setGroups, setRecommendations, setNotice, onOpenRecommendation, onBulkPublished }: Props) {
  ...
  if (!storageReady) return <div className="mt-4 space-y-4"><div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-900"><p className="font-extrabold">Bảng khuyến cáo chưa sẵn sàng trong database.</p><p className="mt-1">Cần áp dụng migration Bảng khuyến cáo trước. Mục nguồn chỉ là provenance, không dùng guideline_entries làm dữ liệu thay thế.</p></div><JsonRecommendationImport guidelineId={guidelineId} user={user} tables={tables} groups={groups} recommendations={recommendations} setTables={setTables} setGroups={setGroups} setRecommendations={setRecommendations} setNotice={setNotice} disabled /></div>;
...
function JsonRecommendationImport({ guidelineId, user, tables, groups, recommendations, setTables, setGroups, setRecommendations, setNotice, disabled = false }: { guidelineId: string; user: User; tables: GuidelineRecommendationTableRecord[]; groups: GuidelineRecommendationGroupRecord[]; recommendations: GuidelineRecommendationRecord[]; setTables: (items: GuidelineRecommendationTableRecord[]) => void; setGroups: (items: GuidelineRecommendationGroupRecord[]) => void; setRecommendations: (items: GuidelineRecommendationRecord[]) => void; setNotice: (notice: Notice) => void; disabled?: boolean }) {
  ...
  async function importJson() {
    if (disabled) { setNotice({ type: "error", text: "Cần áp dụng migration Bảng khuyến cáo trước khi ghi JSON vào database." }); return; }
    try {
