...
const stageLabels: Record<string, string> = { document_analysis: "Đã phân tích, chờ chọn mục", queued: "Đang xếp hàng xử lý", ai_extraction: "AI đang trích xuất", review: "Đang chuẩn bị rà soát", resuming: "Đang tiếp tục xử lý", core_import: "Đang nhập Core", completed: "Hoàn tất", error: "Có lỗi" };
...
export default function AdminGuidelineImportPage({ onNavigate }: { user: User; onNavigate: (path: string) => void }) {
  ...
  async function upload() {
    ...
    try { const result = await uploadGuidelineImport({ file, targetGuidelineId: targetGuidelineId || undefined, sourceLanguage, targetLanguage, preserveEnglishTerminology: preserveTerminology, preserveAbbreviations }); const data = await getGuidelineImportJob(result.job.id); setJobData(data); setSelectedItems(result.items.map((item) => item.id)); await loadJobs(); setNotice({ type: "success", text: "Đã phân tích tài liệu. Hãy chọn các mục cần gửi cho AI." }); }
