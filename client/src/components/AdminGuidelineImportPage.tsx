import { AlertTriangle, Check, FileArchive, FileText, Pause, Play, RefreshCw, Search, Upload, X } from "lucide-react";
...
function guidelineGradeClass(value: string, kind: "class" | "evidence"): string {
  const normalized = value.trim().toLowerCase().replace(/\s+/g, "");
  if (kind === "class") return normalized === "i" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : normalized === "iia" ? "border-sky-200 bg-sky-50 text-sky-800" : normalized === "iib" ? "border-amber-200 bg-amber-50 text-amber-800" : normalized === "iii" ? "border-rose-200 bg-rose-50 text-rose-800" : "border-slate-200 bg-slate-50 text-slate-700";
  return normalized === "a" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : normalized === "b" ? "border-sky-200 bg-sky-50 text-sky-800" : normalized === "c" ? "border-amber-200 bg-amber-50 text-amber-800" : "border-slate-200 bg-slate-50 text-slate-700";
}
...
export default function AdminGuidelineImportPage({ onNavigate }: { user: User; onNavigate: (path: string) => void }) {
  ...
  useEffect(() => {
    ...
    const terminal = new Set(["ready_for_review", "ready_to_import", "completed", "paused", "failed"]);
    const poll = async () => {
      if (disposed || document.visibilityState !== "visible") return;
      try {
        const status = await getGuidelineImportJobStatus(jobId);
        if (disposed) return;
        if (terminal.has(status.status)) {
          const full = await getGuidelineImportJob(jobId);
          if (!disposed) setJobData(full);
          return;
        }
        setJobData((current) => current ? { ...current, job: { ...current.job, ...status } } : current);
      } catch { /* The next scheduled request will retry without disrupting the editor. */ }
    };
    const start = () => { void poll(); if (timer === undefined) timer = window.setInterval(poll, 5_000); };
    const stopWhenHidden = () => { if (document.visibilityState === "visible") start(); };
    start();
    document.addEventListener("visibilitychange", stopWhenHidden);
    return () => { disposed = true; if (timer !== undefined) window.clearInterval(timer); document.removeEventListener("visibilitychange", stopWhenHidden); };
  ...
  const structuralDiagnostics = jobData?.job.analysis_metadata.structuralDiagnostics || [];
  const qualityIssues = Array.from(new Map([
    ...(jobData?.issues || []),
    ...structuralDiagnostics.map((issue, index) => ({ id: `structural-${index}`, recommendation_id: null, severity: issue.severity, issue_code: issue.code, message: issue.message, source_page: issue.sourcePage || null, resolved: false })),
  ].map((issue) => [`${issue.issue_code}|${issue.message}`, issue])).values());
  const blockingIssues = qualityIssues.filter((issue) => ["blocking", "error"].includes(issue.severity) && !issue.resolved);
  ...
  async function importCore() {
    if (!jobData || blockingIssues.length || acceptedRecommendations === 0) return;
...
function JobReview({ data, items, selectedItems, setSelectedItems, visibleRecommendations, search, setSearch, qualityIssues, blockingIssues, busy, onProcess, onResume, onImport, onDelete, onRefresh, onRecommendationChange, onReviewRecommendationTable }: { data: GuidelineImportJobData; items: GuidelineImportItem[]; selectedItems: string[]; setSelectedItems: (items: string[]) => void; visibleRecommendations: GuidelineImportRecommendation[]; search: string; setSearch: (value: string) => void; qualityIssues: GuidelineImportJobData["issues"]; blockingIssues: GuidelineImportJobData["issues"]; busy: boolean; onProcess: () => void; onResume: () => void; onImport: () => void; onDelete: () => void; onRefresh: () => void; onRecommendationChange: (recommendation: GuidelineImportRecommendation) => void; onReviewRecommendationTable: (item: GuidelineImportItem) => void }) {
  ...
  async function reviewAll() {
    if (!window.confirm(`Phê duyệt toàn bộ ${data.recommendations.length} khuyến cáo trong phiên này?`)) return;
    try { await reviewAllGuidelineImportRecommendations(data.job.id); onRefresh(); } catch { /* parent refresh/error path remains available */ }
  }
  return <div className="space-y-4"><section className="rounded-2xl border border-slate-200 bg-white/85 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-[.14em] text-violet-600">{data.document?.original_filename || "Guideline import"}</p><h2 className="mt-1 text-xl font-extrabold text-rose-950">{statusLabels[data.job.status] || data.job.status}</h2><p className="mt-1 text-xs font-semibold text-slate-500">Giai đoạn: {stageLabels[data.job.current_stage] || data.job.current_stage} · Tiến độ: {data.job.progress}% · OCR: {data.document?.ocr_required ? "đã dùng" : "không cần"}</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={onRefresh} className="rounded-lg border border-slate-200 p-2 text-slate-600" title="Làm mới"><RefreshCw size={16} /></button>{["failed", "paused"].includes(data.job.status) && <button type="button" disabled={busy} onClick={onResume} className="inline-flex items-center gap-1 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-extrabold text-amber-800"><Play size={14} />Tiếp tục</button>}<button type="button" disabled={busy || isProcessing} onClick={onDelete} className="rounded-lg border border-rose-200 p-2 text-rose-700" title="Xóa phiên"><X size={16} /></button></div></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><span className="block h-full rounded-full bg-violet-600 transition-all" style={{ width: `${data.job.progress}%` }} /></div></section>
    ...
    {items.length > 0 && data.recommendations.length === 0 && <section className="rounded-2xl border border-violet-200 bg-white/85 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-extrabold text-slate-800">Bảng phát hiện trong tài liệu</h3><p className="mt-1 text-xs font-semibold text-slate-500">Chỉ xử lý Bảng khuyến cáo và Bảng lâm sàng. Mục nguồn chỉ được giữ làm metadata trang/mục, không có trong checklist dịch.</p></div><button type="button" onClick={() => setSelectedItems(selectedItems.length === items.length ? [] : items.map((item) => item.id))} className="text-xs font-extrabold text-violet-700">{selectedItems.length === items.length ? "Bỏ chọn tất cả" : "Chọn tất cả"}</button></div><div className="mt-3 space-y-2">{items.map((item) => <label key={item.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-200 p-3 hover:border-violet-300 hover:bg-violet-50/30"><input type="checkbox" checked={selectedItems.includes(item.id)} disabled={Boolean(item.mandatory)} onChange={(event) => setSelectedItems(event.target.checked ? [...selectedItems, item.id] : items.filter((candidate) => candidate.mandatory || candidate.id !== item.id).map((candidate) => candidate.id).filter((id) => selectedItems.includes(id) || items.find((candidate) => candidate.id === id)?.mandatory))} /><span className="rounded-lg bg-violet-100 p-2 text-violet-700"><FileText size={15} /></span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-slate-800">{itemLabels[item.type]} · {item.label} {item.title ? `— ${item.title}` : ""}</strong><small className="text-xs font-semibold text-slate-500">Trang {item.pageStart || "?"}{item.pageEnd && item.pageEnd !== item.pageStart ? `–${item.pageEnd}` : ""} · {item.resourceType === "recommendation_table" ? "Bảng khuyến cáo bắt buộc" : item.resourceType === "clinical_table" ? `Bảng lâm sàng${item.clinicalTableSubtype ? ` · ${item.clinicalTableSubtype}` : ""}` : item.resourceType === "figure" ? "Figure gốc · caption riêng" : "Tài liệu"}</small></span></label>)}</div><button type="button" disabled={busy || selectedItems.length === 0} onClick={onProcess} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-extrabold text-white disabled:opacity-50"><Play size={16} />Gửi {selectedItems.length} mục cho AI</button></section>}
...
type ImportedTable = {
  ...
  headersVi?: string[];
  rows?: Array<{ cellsOriginal?: string[]; cellsVi?: string[]; groupOrder?: number; rowOrder?: number }>;
  footnotesOriginal?: string[];
  footnotesVi?: string[];
  sourcePage?: number | null;
};

function TranslatedTables({ tables, items, itemStates, busy, onReview }: { tables: Record<string, unknown> | undefined; items: GuidelineImportItem[]; itemStates: Record<string, { status?: string; [key: string]: unknown }> | undefined; busy: boolean; onReview: (item: GuidelineImportItem) => void }) {
  const entries = Object.entries(tables || {}).flatMap(([itemId, value]) => Array.isArray(value)
    ...
    : []).sort((left, right) => (left.table.sourceOrder ?? left.index) - (right.table.sourceOrder ?? right.index) || (left.table.sourcePage ?? Number.MAX_SAFE_INTEGER) - (right.table.sourcePage ?? Number.MAX_SAFE_INTEGER));
  ...
  return <section className="rounded-2xl border border-sky-200 bg-white/85 p-4">
    ...
    <div className="mt-3 space-y-3">
      {entries.map(({ itemId, index, table }) => {
        const item = items.find((candidate) => candidate.id === itemId);
        const state = itemStates?.[itemId];
        const isRecommendationTable = item?.contentType === "recommendation_table";
        const isReviewed = state?.status === "reviewed";
        return <article key={`${itemId}-${index}`} className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50/60 p-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h4 className="font-extrabold text-slate-800">{table.tableNumber || table.sourceTableNumber ? `${table.tableNumber || table.sourceTableNumber} · ` : ""}{table.titleVi || table.titleOriginal || "Bảng chưa có tiêu đề"}</h4>
          <div className="flex items-center gap-2"><span className="text-[11px] font-bold text-slate-500">Trang {table.sourcePage || "?"}</span>{isRecommendationTable && <span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${isReviewed ? "bg-teal-50 text-teal-700" : "bg-amber-50 text-amber-800"}`}>{isReviewed ? "Đã rà soát" : "Bắt buộc rà soát"}</span>}</div>
        </div>
...
function ExtractedFigures({ jobId, figures, onChanged }: { jobId: string; figures: Record<string, GuidelineImportFigure> | undefined; onChanged: () => void }) {
  const entries = Object.entries(figures || {});
  if (!entries.length) return null;
  return <section className="rounded-2xl border border-indigo-200 bg-white/85 p-4">
    <p className="text-xs font-extrabold uppercase tracking-[.14em] text-indigo-700">Figure gốc</p>
    <h3 className="mt-1 text-lg font-extrabold text-slate-800">Caption, crop và quyền sử dụng</h3>
    <p className="mt-1 text-xs font-semibold text-slate-500">Không dịch chữ trong ảnh, không dựng lại Figure và không tự tạo khuyến cáo từ Figure.</p>
    <div className="mt-3 space-y-3">{entries.map(([id, figure]) => <FigureAssetCard key={id} jobId={jobId} figureId={id} figure={figure} onChanged={onChanged} />)}</div>
  </section>;
}

function FigureAssetCard({ jobId, figureId, figure, onChanged }: { jobId: string; figureId: string; figure: GuidelineImportFigure; onChanged: () => void }) {
  const [cropBox, setCropBox] = useState(figure.cropBox || { x: 0.05, y: 0.05, width: 0.9, height: 0.9 });
  ...
  useEffect(() => {
    let active = true;
    let nextUrl = "";
    if (figure.originalAssetPath) void getGuidelineImportFigureAsset(jobId, figureId).then((url) => { nextUrl = url; if (active) setAssetUrl(url); else URL.revokeObjectURL(url); }).catch((error) => { if (active) setNotice(errorText(error)); });
    else setAssetUrl("");
    return () => { active = false; if (nextUrl) URL.revokeObjectURL(nextUrl); };
  }, [jobId, figureId, figure.originalAssetPath]);
  async function crop() {
    setBusy(true); setNotice("");
    try {
      await cropGuidelineImportFigure(jobId, figureId, { pageNumber: figure.sourcePages?.[0] || 1, cropBox });
      onChanged();
    } catch (error) { setNotice(errorText(error)); }
    finally { setBusy(false); }
  }
  return <article className="rounded-xl border border-indigo-100 bg-indigo-50/30 p-3">
    <div className="flex flex-wrap items-start justify-between gap-2"><div><h4 className="font-extrabold text-slate-800">{figure.figureNumber || "Figure"} {figure.translatedTitle || figure.sourceTitle ? `· ${figure.translatedTitle || figure.sourceTitle}` : ""}</h4><p className="mt-1 text-xs font-semibold text-slate-500">Trang {figure.sourcePages?.join(", ") || "?"} · {figure.extractionStatus}</p></div><div className="flex gap-2"><span className="rounded-full bg-white px-2 py-1 text-[10px] font-extrabold text-indigo-700">{figure.permissionStatus}</span><span className="rounded-full bg-white px-2 py-1 text-[10px] font-extrabold text-slate-600">{figure.publicationStatus}</span></div></div>
    {assetUrl && <img src={assetUrl} alt={figure.altText || figure.sourceTitle || "Figure gốc"} className="mt-3 max-h-[30rem] w-auto max-w-full rounded-lg border border-indigo-100 bg-white object-contain" />}
    {figure.sourceCaption && <p className="mt-3 rounded-lg border border-white bg-white/80 p-2 text-xs font-semibold text-slate-700">{figure.sourceCaption}</p>}
    {figure.translatedCaption && <p className="mt-2 rounded-lg border border-teal-100 bg-teal-50/70 p-2 text-xs font-semibold text-teal-800">{figure.translatedCaption}</p>}
    {!figure.originalAssetPath && <div className="mt-3 rounded-lg border border-indigo-100 bg-white/80 p-3"><p className="text-xs font-extrabold text-indigo-800">Crop nguyên bản cho khu vực quản trị</p><p className="mt-1 text-[11px] font-semibold text-slate-500">Nhập vùng theo tỷ lệ trang PDF (0–1). Hệ thống chỉ render ảnh PNG gốc, không sửa chữ, màu hay chú thích bên trong Figure.</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{(["x", "y", "width", "height"] as const).map((key) => <label key={key} className="text-[10px] font-bold text-slate-500">{key}<input type="number" min="0" max="1" step="0.01" value={cropBox[key]} onChange={(event) => setCropBox({ ...cropBox, [key]: Number(event.target.value) })} className="mt-1 h-8 w-full rounded border border-slate-200 px-2 text-xs text-slate-700" /></label>)}</div><button type="button" disabled={busy} onClick={() => void crop()} className="mt-3 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-extrabold text-white disabled:opacity-50">{busy ? "Đang tạo crop..." : "Tạo crop Figure gốc"}</button></div>}
    <p className="mt-2 text-[11px] font-semibold text-amber-800">{figure.originalAssetPath ? "Asset gốc chỉ hiển thị cho quản trị viên/chủ sở hữu; public chỉ có ảnh khi permissionStatus = permission_granted." : "Figure vẫn được giữ trong PDF nguồn; cần crop trước khi dùng nội bộ."}</p>
    {notice && <p className="mt-2 text-xs font-bold text-rose-700">{notice}</p>}
  </article>;
}


function RecommendationReview({ recommendation, onChange }: { recommendation: GuidelineImportRecommendation; onChange: (recommendation: GuidelineImportRecommendation) => void }) { const [saving, setSaving] = useState(false); async function save(patch: Partial<GuidelineImportRecommendation>) { setSaving(true); try { onChange(await updateGuidelineImportRecommendation(recommendation.id, patch)); } finally { setSaving(false); } } return <article className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-extrabold uppercase tracking-[.14em] text-violet-600">{recommendation.source_key} · Trang {recommendation.source_page || "?"}</p><h4 className="mt-1 text-sm font-extrabold text-slate-800">{recommendation.title_original || "Khuyến cáo chưa có tiêu đề"}</h4></div><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${reviewClass(recommendation.review_status)}`}>{recommendation.review_status}</span><span className="rounded-full bg-amber-50 px-2 py-1 text-[10px] font-bold text-amber-700">{recommendation.confidence == null ? "confidence ?" : `confidence ${Math.round(recommendation.confidence * 100)}%`}</span>{recommendation.duplicate_status !== "new" && <span className="rounded-full bg-rose-50 px-2 py-1 text-[10px] font-bold text-rose-700">Trùng: {recommendation.duplicate_status}</span>}</div></div><div className="mt-3 grid gap-3 lg:grid-cols-2"><label className="text-[11px] font-extrabold text-slate-500">Nguyên bản<textarea value={recommendation.recommendation_text_original} onChange={(event) => onChange({ ...recommendation, recommendation_text_original: event.target.value })} onBlur={(event) => void save({ recommendation_text_original: event.target.value })} className="mt-1 min-h-32 w-full rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm font-semibold text-slate-700" /></label><label className="text-[11px] font-extrabold text-teal-700">Tiếng Việt<textarea value={recommendation.recommendation_text_vi} onChange={(event) => onChange({ ...recommendation, recommendation_text_vi: event.target.value })} onBlur={(event) => void save({ recommendation_text_vi: event.target.value })} className="mt-1 min-h-32 w-full rounded-lg border border-teal-200 bg-teal-50/30 p-3 text-sm font-semibold text-slate-700" /></label></div><div className="mt-3 flex flex-wrap items-center gap-2"><input value={recommendation.recommendation_class} onChange={(event) => onChange({ ...recommendation, recommendation_class: event.target.value })} onBlur={(event) => void save({ recommendation_class: event.target.value })} placeholder="Class / strength" className={`h-8 rounded-lg border px-2 text-xs font-extrabold ${guidelineGradeClass(recommendation.recommendation_class, "class")}`} /><input value={recommendation.evidence_level} onChange={(event) => onChange({ ...recommendation, evidence_level: event.target.value })} onBlur={(event) => void save({ evidence_level: event.target.value })} placeholder="Level / LoE" className={`h-8 rounded-lg border px-2 text-xs font-extrabold ${guidelineGradeClass(recommendation.evidence_level, "evidence")}`} /><select disabled={saving} value={recommendation.review_status} onChange={(event) => void save({ review_status: event.target.value as GuidelineImportRecommendation["review_status"] })} className="h-8 rounded-lg border border-slate-200 bg-white px-2 text-xs font-bold"><option value="pending">Chờ duyệt</option><option value="accepted">Chấp nhận</option><option value="needs_review">Cần xem lại</option><option value="rejected">Từ chối</option></select><span className="text-[11px] font-semibold text-slate-400">{saving ? "Đang lưu..." : "Lưu tự động khi rời ô"}</span></div></article>; }
