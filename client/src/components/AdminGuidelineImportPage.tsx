...
export default function AdminGuidelineImportPage({ onNavigate }: { user: User; onNavigate: (path: string) => void }) {
  ...
  const blockingIssues = [...(jobData?.issues.filter((issue) => ["blocking", "error"].includes(issue.severity) && !issue.resolved) || []), ...structuralDiagnostics.map((issue, index) => ({ id: `structural-${index}`, recommendation_id: null, severity: issue.severity, issue_code: issue.code, message: issue.message, source_page: issue.sourcePage || null, resolved: false }))];
