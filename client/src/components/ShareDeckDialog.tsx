...
export default function ShareDeckDialog({ deckId, title, onClose, onShare }: Props) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/35 px-4"><div className="w-full max-w-md rounded-2xl border border-rose-100 bg-white p-6 shadow-xl">
    ...
    <div className="mt-5 rounded-xl bg-teal-50/60 p-3"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-teal-700"><UserRound size={14} /> Đang được chia sẻ với</div>{loadingMembers ? <p className="mt-2 text-xs text-slate-400">Đang tải danh sách...</p> : memberError ? <p className="mt-2 text-xs text-rose-600">{memberError}</p> : members.length === 0 ? <p className="mt-2 text-xs text-slate-400">Chưa có người nhận.</p> : <div className="mt-2 space-y-2">{members.map((member) => <div key={member.user_id} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-sm text-slate-700"><span className="truncate">{member.email}</span><button type="button" title="Dừng chia sẻ" disabled={removing === member.user_id} onClick={() => void remove(member)} className="ml-2 rounded-md p-1.5 text-rose-400 hover:bg-rose-50 hover:text-rose-600 disabled:opacity-40"><UserRoundX size={15} /></button></div>)}</div>}</div>
