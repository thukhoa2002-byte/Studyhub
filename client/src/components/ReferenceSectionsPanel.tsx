...
export default function ReferenceSectionsPanel({ section, onChange, open, onMouseEnter, onMouseLeave }: Props) {
  return (
    <aside className={`reference-sections-popover fixed z-[70] ${open ? "" : "pointer-events-none invisible opacity-0"}`} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave} aria-label="Tài liệu tham khảo">
      <div className="rounded-3xl border border-violet-200/80 bg-white/90 p-4 shadow-[0_18px_50px_rgba(109,40,217,.13)] backdrop-blur-xl">
        ...
        <div className="mt-3 grid gap-2">
          <button type="button" onClick={() => onChange("guidelines")} className={`flex items-center gap-3 rounded-2xl border px-3 py-3 text-left transition ${section === "guidelines" ? "border-teal-200 bg-teal-50 text-teal-700 shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-teal-200 hover:bg-teal-50/60"}`}>
          ...
          <button type="button" onClick={() => onChange("books")} className={`flex items-center gap-3 rounded-2xl border px-3 py-3 text-left transition ${section === "books" ? "border-rose-200 bg-rose-50 text-rose-700 shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-rose-200 hover:bg-rose-50/60"}`}>
