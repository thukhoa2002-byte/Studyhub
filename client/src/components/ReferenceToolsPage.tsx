...
type CalculatorType = "bmi" | "egfr" | "mdrd" | "crcl" | "holliday-segar";
type EgfrFormula = "creatinine" | "cystatin" | "combined";
...
const calculatorOptions: Array<{ id: CalculatorType; label: string; description: string }> = [
  ...
  { id: "mdrd", label: "MDRD", description: "4 biến chuẩn hóa" },
...
export default function ReferenceToolsPage({ user }: { user: User | null }) {
  ...
  const [cystatinC, setCystatinC] = useState("1");
  ...
  const egfrCombined = ageYears > 0 && Number.isFinite(ageYears) && creatinineMgDl > 0 && Number.isFinite(creatinineMgDl) && cystatinCMgL > 0 && Number.isFinite(cystatinCMgL)
    ...
    : null;
  const selectedEgfr = egfrFormula === "creatinine" ? egfr : egfrFormula === "cystatin" ? egfrCystatin : egfrCombined;
  ...
  return <section className="mode-panel mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 xl:px-8" aria-labelledby="reference-tools-title">
    <div className="glass-panel border border-violet-100 bg-white/75 p-5 sm:p-7">
      ...
      {activeTool === "calculator" && <div className="mt-6 rounded-2xl border border-teal-200 bg-teal-50/35 p-4 sm:p-5">
        ...
        {(calculatorType === "egfr" || calculatorType === "mdrd" || calculatorType === "crcl") && <div className="mt-5">{calculatorType === "egfr" && <label className="block text-sm font-bold text-slate-700">Công thức eGFR<select value={egfrFormula} onChange={(event) => setEgfrFormula(event.target.value as EgfrFormula)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 outline-none focus:border-teal-400"><option value="creatinine">CKD-EPI Creatinine 2021</option><option value="cystatin">CKD-EPI Cystatin C 2012</option><option value="combined">CKD-EPI Creatinine-Cystatin C 2012</option></select></label>}<div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><label className="block text-sm font-bold text-slate-700">Tuổi (năm)<input type="number" min="1" max="120" step="1" value={age} onChange={(event) => setAge(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 outline-none focus:border-teal-400" /></label><label className="block text-sm font-bold text-slate-700">Giới tính sinh học<select value={sex} onChange={(event) => setSex(event.target.value as "male" | "female")} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 outline-none focus:border-teal-400"><option value="male">Nam</option><option value="female">Nữ</option></select></label>{(calculatorType !== "egfr" || egfrFormula !== "cystatin") && <UnitValueField label="Creatinine huyết thanh" value={creatinine} unit={creatinineUnit} units={creatinineUnits} onValueChange={setCreatinine} onUnitChange={(nextUnit) => changeUnit(creatinine, creatinineUnit, nextUnit, creatinineUnits, setCreatinine, setCreatinineUnit)} />}{calculatorType === "egfr" && egfrFormula !== "creatinine" && <UnitValueField label="Cystatin C" value={cystatinC} unit={cystatinCUnit} units={cystatinCUnits} onValueChange={setCystatinC} onUnitChange={(nextUnit) => changeUnit(cystatinC, cystatinCUnit, nextUnit, cystatinCUnits, setCystatinC, setCystatinCUnit)} />}{calculatorType === "crcl" && <UnitValueField label="Cân nặng" value={weight} unit={weightUnit} units={weightUnits} onValueChange={setWeight} onUnitChange={(nextUnit) => changeUnit(weight, weightUnit, nextUnit, weightUnits, setWeight, setWeightUnit)} />}</div><div className="mt-5 flex flex-wrap items-end justify-between gap-4 rounded-2xl border border-white bg-white/85 p-4"><div><p className="text-sm font-bold text-slate-500">{calculatorType === "egfr" ? egfrFormula === "creatinine" ? "eGFR · CKD-EPI Creatinine 2021" : egfrFormula === "cystatin" ? "eGFR · CKD-EPI Cystatin C 2012" : "eGFR · CKD-EPI Creatinine-Cystatin C 2012" : calculatorType === "mdrd" ? "eGFR · MDRD 4 biến" : "CrCl · Cockcroft-Gault"}</p><p className="mt-1 text-4xl font-black text-teal-800">{calculatorType === "egfr" ? (selectedEgfr === null ? "—" : selectedEgfr.toFixed(1)) : calculatorType === "mdrd" ? (mdrd === null ? "—" : mdrd.toFixed(1)) : (crcl === null ? "—" : crcl.toFixed(1))}</p></div><p className="text-sm font-semibold text-slate-500">mL/min{calculatorType === "crcl" ? "" : "/1,73 m²"}</p></div><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs font-semibold text-slate-500"><span>Nguồn:</span><a href={formulaSources.niddk} target="_blank" rel="noreferrer" className="text-teal-700 underline hover:text-teal-900">NIDDK · CKD-EPI &amp; MDRD</a><a href={formulaSources.kdigo} target="_blank" rel="noreferrer" className="text-teal-700 underline hover:text-teal-900">KDIGO · CKD guideline</a>{calculatorType === "crcl" && <a href={formulaSources.cockcroftGault} target="_blank" rel="noreferrer" className="text-teal-700 underline hover:text-teal-900">Cockcroft-Gault · PubMed</a>}</div><p className="mt-3 text-xs font-semibold leading-5 text-slate-500">Kết quả chỉ mang tính tham khảo lâm sàng; cần đối chiếu tình trạng người bệnh và hướng dẫn chuyên môn.</p></div>}
      </div>}
