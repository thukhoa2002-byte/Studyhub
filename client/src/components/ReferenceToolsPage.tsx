import { ArrowLeft, Calculator, Check, ChevronDown, ClipboardList, Edit3, FilePlus2, FileUp, FunctionSquare, Plus, Save, Table2, Trash2, UploadCloud, X } from "lucide-react";
...
function UnitValueField({ label, value, unit, units, onValueChange, onUnitChange }: { label: string; value: string; unit: string; units: UnitOption[]; onValueChange: (value: string) => void; onUnitChange: (unit: string) => void }) {
  return <label className="block min-w-0 text-sm font-bold text-slate-700">
    {label}
    <span className="mt-1.5 flex h-11 min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-white focus-within:border-teal-400">
      ...
      <select value={unit} onChange={(event) => onUnitChange(event.target.value)} className="medical-select h-full w-24 shrink-0 border-l border-slate-200 bg-slate-50 px-2 text-sm font-extrabold text-teal-700 outline-none">
...
function AnimatedDropdown({ label, value, options, onChange }: { label: string; value: string; options: Array<{ value: string; label: string }>; onChange: (value: string) => void }) {
  ...
  return <div ref={dropdownRef} className="relative min-w-0">
    <span className="block text-sm font-bold text-slate-700">{label}</span>
    <button type="button" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((current) => !current)} className={`medical-dropdown__trigger medical-select mt-1.5 flex h-11 w-full items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 text-left text-sm font-bold text-slate-700 outline-none ${open ? "medical-dropdown__trigger--open" : ""}`}>
      <span className="truncate">{selectedOption?.label}</span>
      <ChevronDown size={17} className={`shrink-0 text-teal-700 transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
    </button>
    {open && <div className="medical-dropdown__menu absolute left-0 right-0 top-full z-40 mt-2 overflow-hidden rounded-xl border border-teal-100 bg-white p-1.5 shadow-[0_16px_35px_rgba(15,118,110,.18)]" role="listbox" aria-label={label}>
      {options.map((option) => <button key={option.value} type="button" role="option" aria-selected={option.value === value} onClick={() => { onChange(option.value); setOpen(false); }} className={`medical-dropdown__option flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-semibold transition ${option.value === value ? "medical-dropdown__option--selected" : "text-slate-600 hover:bg-teal-50 hover:text-teal-800"}`}><span>{option.label}</span>{option.value === value && <Check size={16} />}</button>)}
    </div>}
  </div>;
}
...
export default function ReferenceToolsPage({ user }: { user: User | null }) {
  ...
  return <section className="mode-panel mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 xl:px-8" aria-labelledby="reference-tools-title">
    <div className="glass-panel border border-violet-100 bg-white/75 p-5 sm:p-7">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{toolGroups.map(({ id, title, description, icon: Icon, className }) => <div key={id} className={`relative flex min-h-0 items-center gap-3 rounded-2xl border p-3 text-left transition hover:-translate-y-0.5 hover:shadow-md ${className} ${((id === "calculator" && activeTool === "calculator") || (id === "data-table" && activeTool === "data-table") || (id === "scores" && activeTool === "score")) ? "ring-2 ring-teal-300" : ""}`}><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/80"><Icon size={19} /></span><span className="min-w-0 pr-8"><strong className="block text-sm font-extrabold leading-5 text-slate-800">{title}</strong><small className="mt-0.5 block text-xs font-semibold leading-4 text-slate-500">{description}</small></span><button type="button" title={`Tạo ${title.toLowerCase()}`} aria-label={`Tạo ${title.toLowerCase()}`} onClick={() => openToolCreator(id)} className="absolute right-2.5 top-2.5 inline-flex h-7 w-7 items-center justify-center rounded-lg bg-white/85 text-slate-600 shadow-sm transition hover:bg-white hover:text-teal-700"><Plus size={16} /></button></div>)}</div>
