...
export default function McqPage({ userId, userEmail, onAiCallsRemaining, section }: Props) {
  ...
  const [folderIcons, setFolderIcons] = useState<Record<string, "folder" | "lung">>({});
  ...
  const decks = useMemo<DeckDefinition[]>(() => [
    ...staticDecks.flatMap<DeckDefinition>((deck) => {
      ...
      return [{
        ...
        questionCount: managedBank.questions.length || managedBank.question_count || 0,
    ...
    ...libraryBanks.filter((item) => item.status !== "archived" && (item.status === "published" || canManage) && !staticBankIds.has(item.id)).map((item) => ({
      ...
      questionCount: item.questions.length || item.question_count || 0,
  ...
  function renderDeckCard(deck: DeckDefinition, panelIndex?: number, animateIn = false): ReactNode {
    return <article key={deck.key} draggable={canManage} onDragStart={(event) => startMcqDrag(event, { kind: "bank", id: deck.key })} onDragEnd={() => { setDraggedMcqItem(null); setDropFolderId(null); }} style={animateIn ? { animationDelay: `${(panelIndex || 0) * 50}ms` } : undefined} className={`group relative flex min-h-36 flex-col rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50/90 via-white to-teal-50/80 p-3 text-left shadow-sm transition hover:border-violet-300 hover:shadow-md sm:p-4 ${animateIn ? "mcq-folder-panel__deck" : ""}`}>
        ...
        <div className="flex min-w-0 items-center gap-2"><div draggable={canManage} onDragStart={(event) => startMcqDrag(event, { kind: "bank", id: deck.key })} onDragEnd={() => { setDraggedMcqItem(null); setDropFolderId(null); }} title={canManage ? "Kéo bộ MCQ vào thư mục" : undefined} className="flex h-10 w-10 shrink-0 cursor-grab items-center justify-center rounded-xl bg-violet-100 text-violet-700 active:cursor-grabbing"><McqIcon size={22} /></div><div className="flex h-10 min-w-0 flex-col justify-center"><h2 className="truncate text-lg font-extrabold leading-5 text-rose-950" title={deck.title}>{deck.title}</h2><p className="truncate text-xs font-semibold leading-4 text-slate-400">{deck.questionCount ?? 0} câu</p></div></div>
      </div>
      <p className="mt-2 text-xs leading-4 text-slate-500">{deck.description}</p>
      <div className="mt-auto flex flex-wrap items-end justify-between gap-2 pt-2">
        {canManage && <div className="pointer-events-none relative flex items-center gap-1 opacity-0 transition duration-200 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100">
          <button type="button" aria-label={"Sửa nội dung " + deck.title} title="Sửa nội dung bộ trắc nghiệm" onClick={() => void requestDeckEdit(deck)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-violet-100 bg-white/95 text-violet-700 shadow-sm transition hover:bg-violet-50"><Pencil size={16} /></button>
          <button type="button" aria-label={deck.visibility === "published" ? "Chuyển thành riêng tư" : "Công khai bộ trắc nghiệm"} title={deck.visibility === "published" ? "Chuyển riêng tư" : "Công khai bộ trắc nghiệm"} onClick={() => void changeDeckVisibility(deck, deck.visibility === "published" ? "draft" : "published")} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-teal-100 bg-white/95 text-teal-700 shadow-sm transition hover:bg-teal-50">{deck.visibility === "published" ? <LockKeyhole size={16} /> : <Globe2 size={16} />}</button>
          <button type="button" aria-label={"Xóa " + deck.title} title="Xóa bộ trắc nghiệm" onClick={() => void removeDeck(deck)} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-rose-100 bg-white/95 text-rose-600 shadow-sm transition hover:bg-rose-50"><Trash2 size={16} /></button>
        </div>}
        {canManage ? <div className="relative hidden">
          ...
          {openDeckMenuId === deck.key && <div className="absolute bottom-full left-0 z-50 mb-2 w-64 rounded-2xl border border-slate-200 bg-white p-2 text-left shadow-[0_18px_40px_rgba(15,23,42,.2)]">
            ...
            <label className="mt-1 block rounded-xl px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50">Chuyển vào thư mục<AnimatedDropdown value={deck.folderId || ""} options={[{ value: "", label: "Thư mục gốc" }, ...libraryFolders.map((folder) => ({ value: folder.id, label: `${folder.parent_id ? "↳ " : ""}${folder.title}` }))]} onChange={(value) => void moveDeckToFolder(deck, value || null)} ariaLabel="Chuyển vào thư mục" triggerClassName="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm font-semibold text-slate-700" /></label>
        ...
        <div className="pointer-events-none flex items-center justify-end gap-1.5 text-xs font-bold opacity-0 transition duration-200 group-hover:pointer-events-auto group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:opacity-100"><button type="button" onClick={() => void openPreview(deck)} className="inline-flex items-center gap-1 rounded-lg border border-violet-200 bg-white px-2.5 py-1.5 text-violet-700 hover:bg-violet-50"><Eye size={14} />Xem trước</button><RippleButton text="Bắt đầu" icon={<Play size={14} fill="currentColor" />} onClick={() => openDeck(deck)} className="px-2.5 py-1.5 text-xs font-bold" /></div>
      </div>
    </article>;
  }
  ...
  function renderFolder(folder: McqFolder, depth = 0, includeDecks = true): ReactNode {
    ...
    const selectedIcon = folderIcons[folder.id] || "folder";
    const iconOptions: Array<{ id: "folder" | "lung"; label: string; icon: ReactNode }> = [
      { id: "folder", label: "Thư mục", icon: <Folder size={22} /> },
      { id: "lung", label: "Phổi", icon: <LungIcon size={22} /> },
    ];
    return <div key={folder.id} className={depth === 0 ? "mcq-folder-tree min-w-0" : "min-w-0 rounded-2xl border border-amber-200/80 bg-white/70 p-3 transition duration-200 hover:-translate-y-0.5 hover:border-amber-300 hover:shadow-md"}>
      ...
      >
        <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><button type="button" onClick={(event) => { event.stopPropagation(); setIconPickerFolderId((current) => current === folder.id ? null : folder.id); }} aria-label={`Mở bộ sưu tập icon cho ${folder.title}`} title="Mở bộ sưu tập icon" className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-700 transition hover:bg-amber-200 ${iconPickerFolderId === folder.id ? "ring-2 ring-teal-300" : ""}`}>{selectedIcon === "lung" ? <LungIcon size={22} /> : <Folder size={20} />}</button><button type="button" onMouseEnter={() => setHoveredFolderId(folder.id)} onClick={() => { setActiveFolderId(folder.id); setHoveredFolderId(folder.id); setOpenDeckMenuId(null); }} className={`min-w-0 text-left ${panelFolderId === folder.id ? "text-amber-900" : ""}`}><span className="block truncate text-base font-extrabold text-slate-800">{folder.title}</span><span className="block text-xs font-semibold text-slate-500">{folderDecks.length} bộ trắc nghiệm</span></button></div>{canManage && <div className="flex shrink-0 items-center gap-1"><button type="button" disabled={folderActionBusy === folder.id} aria-label={`Đổi tên thư mục ${folder.title}`} title="Đổi tên thư mục" onClick={() => beginEditFolder(folder)} className="rounded-xl p-2 text-violet-700 hover:bg-violet-100 disabled:opacity-45"><Pencil size={17} /></button><button type="button" disabled={folderActionBusy === folder.id} aria-label={`Di chuyển thư mục ${folder.title}`} title="Di chuyển thư mục" onClick={() => beginEditFolder(folder)} className="rounded-xl p-2 text-amber-700 hover:bg-amber-100 disabled:opacity-45"><FolderInput size={17} /></button><button type="button" disabled={folderActionBusy === folder.id} aria-label={folder.status === "published" ? `Chuyển ${folder.title} thành riêng tư` : `Công khai ${folder.title}`} title={folder.status === "published" ? "Chuyển riêng tư" : "Công khai thư mục"} onClick={() => void toggleFolderVisibility(folder)} className="rounded-xl p-2 text-teal-700 hover:bg-teal-100 disabled:opacity-45">{folder.status === "published" ? <LockKeyhole size={17} /> : <Globe2 size={17} />}</button><button type="button" disabled={folderActionBusy === folder.id} aria-label={`Xóa thư mục ${folder.title}`} title="Xóa thư mục" onClick={() => void removeFolder(folder)} className="rounded-xl p-2 text-rose-600 hover:bg-rose-100 disabled:opacity-45"><Trash2 size={17} /></button></div>}</div>
        {canManage && iconPickerFolderId === folder.id && <div onClick={(event) => event.stopPropagation()} className="mt-3 rounded-2xl border border-teal-200 bg-white/85 p-3 shadow-sm"><p className="text-[11px] font-extrabold uppercase tracking-[.14em] text-teal-700">Bộ sưu tập icon</p><div className="mt-2 flex flex-wrap gap-2">{iconOptions.map((option) => <button key={option.id} type="button" onClick={() => { setFolderIcons((items) => ({ ...items, [folder.id]: option.id })); setIconPickerFolderId(null); }} aria-label={`Dùng icon ${option.label} cho ${folder.title}`} title={option.label} className={`flex h-11 w-11 items-center justify-center rounded-xl border transition ${selectedIcon === option.id ? "border-teal-300 bg-teal-50 text-teal-700 ring-2 ring-teal-100" : "border-slate-200 bg-slate-50 text-slate-600 hover:border-teal-200 hover:bg-teal-50"}`}>{option.icon}</button>)}</div></div>}
        {editingFolderId === folder.id && <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_220px_auto_auto]"><input value={editingFolderTitle} onChange={(event) => setEditingFolderTitle(event.target.value)} className="rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm font-semibold" placeholder="Tên thư mục" /><AnimatedDropdown value={editingFolderParentId || ""} options={[{ value: "", label: "Thư mục gốc" }, ...parentOptions.map((item) => ({ value: item.id, label: item.title }))]} onChange={(value) => setEditingFolderParentId(value || null)} ariaLabel="Thư mục cha" triggerClassName="h-10 rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm font-semibold" /><button type="button" disabled={folderActionBusy === folder.id} onClick={() => void saveFolderEdit(folder)} className="rounded-lg bg-amber-500 px-3 py-2 text-sm font-bold text-white disabled:opacity-45">Lưu</button><button type="button" onClick={() => setEditingFolderId(null)} className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600"><X size={16} /></button></div>}
      </div>
        ...
        {editableDecks.map((deck) => <div key={deck.key} className="flex min-w-0 items-center gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-3 transition hover:border-violet-300 hover:bg-violet-50/40">
          ...
          <div className="relative shrink-0">
            ...
            {openDeckMenuId === deck.key && <div className="absolute right-0 top-full z-50 mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 text-left shadow-[0_18px_40px_rgba(15,23,42,.2)]">
              <label className="block rounded-xl px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50">Chuyển vào thư mục<AnimatedDropdown value={deck.folderId || ""} options={[{ value: "", label: "Thư mục gốc" }, ...libraryFolders.map((folder) => ({ value: folder.id, label: `${folder.parent_id ? "↳ " : ""}${folder.title}` }))]} onChange={(value) => void moveDeckToFolder(deck, value || null)} ariaLabel="Chuyển vào thư mục" triggerClassName="mt-1 h-9 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-sm font-semibold text-slate-700" /></label>
            </div>}
          </div>
  ...
  if (!opened) return (
    <section className="mode-panel mx-auto w-full max-w-[1600px] px-5 py-8" aria-labelledby="mcq-title">
      {section === "create" && <>
        <div className="relative z-30 grid items-stretch gap-5 lg:grid-cols-[minmax(0,0.84fr)_minmax(0,1.16fr)]">
          <McqAccessPanel userEmail={userEmail} visible={Boolean(userId)} />
          {renderEditableDecks()}
        </div>
        {(adminAccessReady || isOwner) && canManage && userId && <div id="mcq-admin-studio" className="relative z-0 mt-5"><McqAdminStudio userId={userId} drafts={libraryBanks} requestedBank={requestedEditBank} onChanged={refreshLibrary} onAiCallsRemaining={onAiCallsRemaining} showDrafts={false} /></div>}
      </>}
