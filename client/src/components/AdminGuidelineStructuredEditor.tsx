...
function GuidelineWorkspace({ user, guidelineId, initialTab, onNavigate }: { user: User; guidelineId: string; initialTab: Tab; onNavigate: (path: string) => void }) {
  ...
  return <section>
    ...
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-violet-100 bg-white/80 p-3"><div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${statusClass(document.status)}`}>{statusLabels[document.status]}</span><span className="text-xs font-semibold text-slate-400">UUID giữ nguyên: {document.id}</span></div><div className="flex flex-wrap gap-2">{document.status === "draft" && <><button type="button" onClick={() => void changeStatus("in_review")} className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-extrabold text-amber-800">Gửi rà soát</button><button type="button" onClick={() => window.confirm("Xuất bản Guideline theo chính sách hiện tại?") && void changeStatus("published")} className="rounded-xl bg-teal-600 px-3 py-2 text-xs font-extrabold text-white">Xuất bản</button><button type="button" onClick={() => void removePermanently()} className="ml-2 inline-flex items-center gap-1 rounded-xl border border-rose-200 bg-white px-3 py-2 text-xs font-extrabold text-rose-700"><Trash2 size={14} />Xóa</button></>}{(document.status === "in_review") && <><button type="button" onClick={() => void changeStatus("draft")} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-extrabold text-slate-700">Trả về nháp</button><button type="button" onClick={() => window.confirm("Xuất bản Guideline theo chính sách hiện tại?") && void changeStatus("published")} className="rounded-xl bg-teal-600 px-3 py-2 text-xs font-extrabold text-white">Xuất bản</button></>}{document.status === "published" && <button type="button" onClick={() => window.confirm("Lưu trữ Guideline này? Nội dung sẽ không còn công khai.") && void changeStatus("archived")} className="inline-flex items-center gap-1 rounded-xl border border-rose-200 bg-white px-3 py-2 text-xs font-extrabold text-rose-700"><Archive size={14} />Lưu trữ</button>}{document.status === "archived" && <><button type="button" onClick={() => void changeStatus("draft")} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-extrabold text-slate-700">Khôi phục về nháp</button><button type="button" onClick={() => window.confirm("Xuất bản lại Guideline? Hệ thống sẽ kiểm tra toàn bộ điều kiện.") && void changeStatus("published")} className="rounded-xl bg-teal-600 px-3 py-2 text-xs font-extrabold text-white">Xuất bản lại</button><button type="button" onClick={() => void removePermanently()} className="ml-2 inline-flex items-center gap-1 rounded-xl border border-rose-200 bg-white px-3 py-2 text-xs font-extrabold text-rose-700"><Trash2 size={14} />Xóa vĩnh viễn</button></>}</div></div>
...
function SectionRow({ section, sections, setSections, setNotice, onMoveUp, onMoveDown, isFirst, isLast }: { section: GuidelineSectionRecord; sections: GuidelineSectionRecord[]; setSections: (items: GuidelineSectionRecord[]) => void; setNotice: (notice: Notice) => void; onMoveUp: () => void; onMoveDown: () => void; isFirst: boolean; isLast: boolean }) {
  ...
  async function republish() { try { const updated = await republishGuidelineSection(section.id); setSections(sections.map((item) => item.id === updated.id ? updated : item)); setNotice({ type: "success", text: "Đã xuất bản lại section." }); } catch (error) { setNotice({ type: "error", text: errorText(error) }); } }
