...
export function buildGuidelineImportCandidate(bundle: GuidelineTableBundle, sourceType: GuidelineImportCandidate["sourceType"], existingDrugs: Drug[]): GuidelineImportCandidate {
  ...
  return { candidateId: `guideline-import-${Date.now()}`, sourceType, guideline, table: bundle.table, commonGuidance, rows, drugCandidates, provenance, validationErrors: errors, validationWarnings: warnings, importStatus: errors.length ? "invalid" : "ready" };
}
