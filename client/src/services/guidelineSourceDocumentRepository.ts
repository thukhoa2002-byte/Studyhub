...
const sourceDocumentColumns = "id,guideline_id,owner_id,original_filename,storage_path,mime_type,source_kind,checksum,page_count,extraction_status,created_at,updated_at";

export async function listGuidelineSourceDocuments(guidelineId: string): Promise<GuidelineSourceDocumentRecord[]> {
  const query = requireGuidelineClient().from("guideline_source_documents").select(sourceDocumentColumns).eq("guideline_id", guidelineId).order("created_at", { ascending: true }).limit(100);
...
export async function createGuidelineSourceDocument(ownerId: string, input: Omit<GuidelineSourceDocumentRecord, "id" | "owner_id" | "created_at" | "updated_at">): Promise<GuidelineSourceDocumentRecord> {
  const { data, error } = await requireGuidelineClient().from("guideline_source_documents").insert({ ...input, owner_id: ownerId }).select(sourceDocumentColumns).single();
  if (error) throw error;
  return data as GuidelineSourceDocumentRecord;
}
...
export async function updateGuidelineSourceDocument(id: string, patch: Partial<Pick<GuidelineSourceDocumentRecord, "original_filename" | "mime_type" | "source_kind" | "checksum" | "page_count" | "extraction_status">>): Promise<GuidelineSourceDocumentRecord> {
  const { data, error } = await requireGuidelineClient().from("guideline_source_documents").update({ ...patch, updated_at: new Date().toISOString() }).eq("id", id).select(sourceDocumentColumns).single();
  if (error) throw error;
  return data as GuidelineSourceDocumentRecord;
}
