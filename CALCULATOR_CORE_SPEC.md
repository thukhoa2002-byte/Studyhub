...
`CalculatorDefinition` is the only runtime model. It contains identity and lifecycle metadata, localized title/description/purpose, specialty/category, input definitions, `calculation.handlerId`, optional scoring rules, result definitions, references, Guideline references, persistent test cases, and audit timestamps.
...
- **Equation:** a registered handler such as `bmi` or `cockcroft-gault`.
